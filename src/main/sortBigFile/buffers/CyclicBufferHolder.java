 ...
 * Threads may concurrently get buffers and after trySpinWait have to return it to CyclicBufferHolder
 *
...
public class CyclicBufferHolder<T> {
    ...
    static class CyclicBuffer<T> implements ICyclicBuffer<T> {
        ...
        public T pull() {
            T value = getFirst();
            array[indexStrategy.getIndex(head) + startPointer] = null;//Let's GC do it's trySpinWait
            head++;
